...
import { authApi, eraApi, presetMainStoryApi, presetScriptApi, worldApi, characterApi, scriptApi, systemMainStoryApi } from '../../services/api';
...
describe('初始化流程测试', () => {
  ...
  describe('测试用例1: 注册用户并执行初始化，验证体验场景不显示', () => {
    it('应该注册新用户并完成初始化，初始化后不应看到体验场景', async () => {
      ...
      const presetScripts = await presetScriptApi.getByEraId(mockMyUniversityEra.id);
      expect(presetScripts).toBeDefined();
      expect(Array.isArray(presetScripts)).toBe(true);
      ...
      const userScripts = await scriptApi.getAllScripts(mockToken);
      ...
      const userWorldScenes = eras.map(era => ({
        ...
        characters: [],
        scripts: userScripts.filter(s => (s as any).eraId === era.id).map(s => ({
          id: s.id.toString(),
          title: s.title,
          content: s.content,
  ...
  describe('测试用例2: 验证初始化后可以正常获取剧情', () => {
    it('应该能够获取初始化后创建的场景和剧本', async () => {
      ...
      // 验证剧本与场景的关联
      const eraScripts = scripts.filter(s => (s as any).eraId === mockUserEra.id);
