...
export class SkillService {
   ...
   * 
   * 注意：此方法只返回有 function_schema 的技能（用于 Function Calling）
   * 如需获取所有技能（包括提示词驱动技能），请使用 getCharacterAllSkills
   */
  ...
  /**
   * 获取角色所有已装备技能（包括 Function Calling 和提示词驱动技能）
   * 返回包含两类技能的完整信息
   */
  async getCharacterAllSkills(
    characterId: number, 
    token?: string
  ): Promise<{
    functionCallingSkills: FunctionDefinition[];
    promptDrivenSkills: Array<{ skillId: string; name: string; description: string }>;
  }> {
    try {
      const authToken = token || localStorage.getItem('token');
      ...
      const response = await fetch(`${this.baseUrl}/character/${characterId}/all`, {
        method: 'GET',
        headers,
      });
      ...
      return result.data || { functionCallingSkills: [], promptDrivenSkills: [] };
    } catch (error) {
      console.error('[SkillService] 获取角色所有技能失败:', error);
      // 如果新API失败，fallback到旧的API（只返回Function Calling技能）
      const functionCallingSkills = await this.getCharacterAvailableSkills(characterId, token);
      return {
