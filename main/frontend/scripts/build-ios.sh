...
# 检查操作系统（iOS 构建需要 macOS）
if [[ "$(uname -s)" != "Darwin" ]]; then
    echo -e "${RED}❌ 错误: iOS 构建需要在 macOS 系统上运行${NC}"
    echo -e "${YELLOW}提示: 当前系统是 $(uname -s)，无法构建 iOS 应用${NC}"
    exit 1
fi
...
if ! command -v xcodebuild &> /dev/null; then
    echo -e "${YELLOW}⚠️  警告: 未找到 Xcode，可能无法在 Xcode 中打开项目${NC}"
    echo -e "${YELLOW}提示: 请安装 Xcode 以进行 iOS 开发和构建${NC}"
else
    XCODE_VERSION=$(xcodebuild -version 2>&1 | head -1)
    echo -e "${GREEN}✅ $XCODE_VERSION${NC}"
fi

# 检查 CocoaPods（可选，Capacitor 会自动处理）
if command -v pod &> /dev/null; then
    POD_VERSION=$(pod --version)
    echo -e "${GREEN}✅ CocoaPods 版本: $POD_VERSION${NC}"
else
    echo -e "${YELLOW}⚠️  提示: CocoaPods 未安装，Capacitor 会自动处理依赖${NC}"
fi
...
if [ ! -d "node_modules/@capacitor/cli" ]; then
    ...
    npm install @capacitor/cli @capacitor/core @capacitor/ios --save
fi

# 检查 iOS 目录
if [ ! -d "ios" ]; then
    echo -e "${YELLOW}⚠️  警告: ios 目录不存在，正在添加 iOS 平台...${NC}"
    npx cap add ios
    echo -e "${GREEN}✅ iOS 平台已添加${NC}"
fi
...
echo -e "${BLUE}💾 步骤 3/5: 准备 iOS 构建...${NC}"
...
# 4. 将 mobile.html 复制为 index.html（供 iOS 使用）
echo -e "${YELLOW}将 mobile.html 复制为 index.html（iOS 专用）...${NC}"
cp dist/mobile.html dist/index.html
echo -e "${GREEN}✅ 复制完成${NC}"
echo ""

# 5. 复制到 iOS（使用 copy 避免无 Xcode 时 pod install 失败）
echo -e "${BLUE}🔄 步骤 4/5: 复制到 iOS 项目...${NC}"
if ! npx cap copy ios; then
    echo -e "${RED}❌ 错误: iOS 复制失败${NC}"
...
# 若需要更新原生依赖，可再运行: npx cap sync ios
echo -e "${GREEN}✅ 复制完成${NC}"
echo ""
...
echo -e "   1. 打开 Xcode: ${YELLOW}npm run cap:open:ios${NC}"
echo -e "   2. 在 Xcode 中选择设备并运行"
echo -e "   3. 或使用命令行运行: ${YELLOW}npm run cap:run:ios${NC}"
echo ""
