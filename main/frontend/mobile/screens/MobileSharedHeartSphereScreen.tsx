  ...
  return (
    <div className="h-full bg-black flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto pt-20 pb-[calc(4rem+env(safe-area-inset-bottom))] px-4" style={{ WebkitOverflowScrolling: 'touch' }}>
        <div className="mb-6">
          <h1 className={`text-2xl font-bold ${MobileColors.text.primary} mb-2`}>共享心域</h1>
          <p className={`${MobileColors.text.muted} text-sm`}>选择场景开始探索</p>
        </div>
          ...
          <div className="space-y-4">
            {scenes.map(scene => (
              <div
                ...
                className={`relative h-48 w-full rounded-2xl overflow-hidden ${MobileColors.border.default} ${MobileCardStyles.shadow} ${MobileCardStyles.interactive}`}
                role="button"
                tabIndex={0}
                onKeyPress={(e) => {
              ...
              >
                ...
                <div className={`absolute top-4 right-4 bg-black/40 backdrop-blur-md px-2 py-1 rounded-full ${MobileColors.border.default} text-[10px] ${MobileColors.text.primary}`}>
                  进入 &rarr;
                </div>
              </div>
            ))}
