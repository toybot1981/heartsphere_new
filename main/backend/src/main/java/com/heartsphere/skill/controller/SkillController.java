...
import com.heartsphere.skill.dto.CharacterAvailableSkillsDTO;
...
public class SkillController {
    ...
    public ResponseEntity<ApiResponse<CharacterAvailableSkillsDTO>> getCharacterAllSkills(
            @PathVariable Long characterId,
            Authentication authentication) {
        ...
        // 分离两类技能
        List<FunctionDefinitionDTO> functionCallingSkills = skillRegistry.toFunctionDefinitions(skills)
            .stream()
            .map(fd -> FunctionDefinitionDTO.builder()
                .name(fd.getName())
                .description(fd.getDescription())
        ...
        CharacterAvailableSkillsDTO result = 
            CharacterAvailableSkillsDTO.builder()
                .functionCallingSkills(functionCallingSkills)
