...
/**
 * 全局异常处理器（Main项目专用）
 * 注意：为避免与shared模块的GlobalExceptionHandler冲突，类名改为MainGlobalExceptionHandler
 * Spring会自动使用类名（camelCase）作为bean名称，因此bean名称为mainGlobalExceptionHandler，不会与shared模块的globalExceptionHandler冲突
 */
@Slf4j
