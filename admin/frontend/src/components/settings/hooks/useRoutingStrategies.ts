...
import type { AIModelConfig, RoutingStrategy } from '../../../services/api/admin/types';
...
export const useRoutingStrategies = (
    ...
    const loadRoutingStrategies = useCallback(async () => {
        ...
        try {
            ...
            setRoutingStrategies(data as RoutingStrategy[]);
        } catch (error: any) {
            showAlert(
    ...
    const handleLoadStrategy = async (capability: 'text' | 'image' | 'audio' | 'video') => {
        ...
        try {
            ...
            setStrategyFormData({ ...strategy } as Partial<RoutingStrategy>);
            setEditingStrategy({ ...strategy, capability } as RoutingStrategy);
