...
import type { ScenarioEvent } from '../services/api';
...
export const EventsManagement: React.FC<EventsManagementProps> = ({
    ...
    const loadEvents = async () => {
        ...
        setLoading(true);
        try {
            // 使用 entity API 加载所有事件
            const response = await adminEntityApi.getEvents(adminToken, selectedEraId || undefined);
    ...
    const handleCreate = async () => {
        ...
        try {
            const createData = {
                ...
                systemEraId: formData.eraId || undefined, // 如果使用系统场景
                iconUrl: formData.iconUrl || undefined,
                tags: formData.tags || undefined,
                sortOrder: formData.sortOrder,
                isActive: formData.isActive,
            };
            await adminEntityApi.createEvent(createData, adminToken);
            await loadEvents();
            resetForm();
    ...
    const handleUpdate = async () => {
        ...
        try {
            if (!editingEvent.id) {
                showAlert('事件ID不能为空', '缺少参数', 'warning');
                return;
            }
            ...
            const updateData = {
                ...
                description: formData.description || undefined,
                eraId: formData.eraId ?? undefined,
                systemEraId: formData.eraId ?? undefined,
                iconUrl: formData.iconUrl || undefined,
                tags: formData.tags || undefined,
            ...
            // 确保 id 是 number 类型
            const eventId = typeof editingEvent.id === 'string' ? parseInt(editingEvent.id, 10) : editingEvent.id;
            if (isNaN(eventId)) {
                showAlert('无效的事件ID', '参数错误', 'error');
                return;
            }
            
            await adminEntityApi.updateEvent(eventId, updateData, adminToken);
            await loadEvents();
            resetForm();
    ...
    const handleDelete = async (event: ScenarioEvent) => {
        ...
        try {
            ...
            await adminEntityApi.deleteEvent(eventId, adminToken);
            await loadEvents();
