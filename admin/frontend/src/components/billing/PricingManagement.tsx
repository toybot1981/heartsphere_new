...
export const PricingManagement: React.FC<PricingManagementProps> = ({
  ...
  const groupPricingsByProvider = () => {
    ...
    pricings.forEach(pricing => {
      ...
      let providerDisplayName: string = '未知提供商';
    ...
    return Object.values(grouped).sort((a, b) => {
      // 类型守卫：检查是否是 AIProvider 类型
      const providerA = a.provider as AIProvider | { name: string; displayName: string };
      const providerB = b.provider as AIProvider | { name: string; displayName: string };
      
      const nameA = ('name' in providerA ? providerA.name : 'unknown').toLowerCase();
      const nameB = ('name' in providerB ? providerB.name : 'unknown').toLowerCase();
      ...
      // 如果优先级相同，按显示名称排序
      const getDisplayName = (p: AIProvider | { name: string; displayName: string }): string => {
        if ('displayName' in p && p.displayName) {
      ...
      const displayNameA = getDisplayName(providerA);
      const displayNameB = getDisplayName(providerB);
    ...
    <div className="max-w-7xl mx-auto">
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {groupPricingsByProvider().map((item, index) => {
            const provider: AIProvider | { name: string; displayName: string } = item.provider;
            const providerPricings = item.pricings;
            
            // 类型安全地获取 providerKey
            const providerKey = 'id' in provider && provider.id !== undefined
              ? (typeof provider.id === 'number' ? provider.id : String(provider.id))
              : ('name' in provider && provider.name ? provider.name : `provider-${index}`);
            
            // 类型安全地获取 displayName
            const displayName = ('displayName' in provider && provider.displayName)
              ? provider.displayName
              : ('name' in provider && provider.name ? provider.name : '未知提供商');
            
            // 类型安全地获取 enabled
            const enabled = 'enabled' in provider && provider.enabled !== undefined
              ? provider.enabled
              : true;
