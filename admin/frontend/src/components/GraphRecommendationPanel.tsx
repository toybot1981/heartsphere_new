...
import { adminGraphRecommendationApi } from '../services/api';
import type {
...
export const GraphRecommendationPanel: React.FC<GraphRecommendationPanelProps> = ({
  ...
  const extractEntities = (): Array<Record<string, any>> => {
    return nodes
      ...
      .map((node) => {
        ...
        return {
          entityId: config.eraId || config.characterId || config.eventId || config.itemId,
          entityType: node.nodeType,
          entityName: config.eraName || config.characterName || config.eventName || config.itemName,
          eraId: config.eraId,
        };
      })
      .filter((e) => e.entityId);
  ...
  const extractContext = (): Record<string, any> => {
    ...
    // 提取场景ID
    const eraNode = nodes.find((n) => n.nodeType === 'era' && n.nodeConfig?.eraId);
    if (eraNode?.nodeConfig?.eraId) {
      context.eraId = eraNode.nodeConfig.eraId;
      // 如果场景节点有worldId，也提取世界ID
      if (eraNode.nodeConfig.worldId) {
        context.worldId = eraNode.nodeConfig.worldId;
      }
    }
