...
export const MainStoriesManagement: React.FC<MainStoriesManagementProps> = ({ 
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const generated = await aiService.generateMainStory({
                ...
                characterIds: eraCharacters.map(c => c.id).filter((id): id is number => id !== undefined),
    ...
    const handleGenerateBackground = async () => {
        ...
        try {
            ...
            if (response && response.url) {
                // 直接使用返回的URL作为背景图片
                setFormData({...formData, backgroundUrl: response.url});
                showAlert('背景生成成功！', '成功', 'success');
            } else {
                throw new Error('未获取到图片URL');
            }
        } catch (err: any) {
