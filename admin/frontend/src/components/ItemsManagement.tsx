...
export const ItemsManagement: React.FC<ItemsManagementProps> = ({
    ...
    const loadItems = async () => {
        ...
        setLoading(true);
        try {
            // 使用 entity API 加载所有物品
            const response = await adminEntityApi.getItems(adminToken, selectedEraId || undefined);
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            await adminEntityApi.createItem(createData, adminToken);
            await loadItems();
            resetForm();
    ...
    const handleUpdate = async () => {
        ...
        try {
            if (!editingItem.id) {
                showAlert('物品ID不能为空', '缺少参数', 'warning');
                return;
            }
            
            const updateData: Partial<ScenarioItem> = {
                name: formData.name || '',
                itemId: formData.itemId || '',
                description: formData.description || undefined,
                eraId: formData.eraId ?? undefined,
            ...
            // 确保 id 是 number 类型
            const itemId = typeof editingItem.id === 'string' ? parseInt(editingItem.id, 10) : editingItem.id;
            if (isNaN(itemId)) {
                showAlert('无效的物品ID', '参数错误', 'error');
                return;
            }
            
            await adminEntityApi.updateItem(itemId, updateData, adminToken);
            await loadItems();
            resetForm();
    ...
    const handleDelete = async (item: ScenarioItem) => {
        ...
        try {
            ...
            await adminEntityApi.deleteItem(itemId, adminToken);
            await loadItems();
