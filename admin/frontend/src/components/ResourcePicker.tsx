...
interface ResourcePickerProps {
  category: 'avatar' | 'character' | 'era' | 'scenario' | 'journal' | 'general' | 'background';
  onSelect: (url: string) => void;
  onClose: () => void;
  currentUrl?: string;
  token?: string | null;
  useAdminApi?: boolean;  // 是否使用管理员 API，已废弃但保留兼容性
}
...
export const ResourcePicker: React.FC<ResourcePickerProps> = ({ 
  ...
  currentUrl,
  token,
  useAdminApi = false  // 默认使用 adminApi，此参数已废弃
}) => {
  ...
  useEffect(() => {
    const loadResources = async () => {
      ...
      try {
        const data = await adminApi.resources.getAll(token, category);
