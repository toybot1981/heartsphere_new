...
export function constructCharacterBackgroundPrompt(
  ...
  const styleSuffix = worldStyle ? getStylePromptSuffix(worldStyle) : 'Style: Modern Chinese Anime (Manhua), cinematic lighting, epic atmosphere.';
  return `Beautiful horizontal background scene for character ${name}. Description: ${bio}. ${styleSuffix} Background matching theme color ${themeColor}, atmospheric and immersive.`;
}
