...
export const adminMainStoriesApi = {
  ...
  getById: (id: number, token: string): Promise<SystemMainStory> => {
    return request<SystemMainStory>(`/system/main-stories/${id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
  ...
  getByEraId: (eraId: number, token: string): Promise<SystemMainStory> => {
    return request<SystemMainStory>(`/system/main-stories/era/${eraId}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
  ...
  delete: (id: number, token: string): Promise<void> => {
    return request<void>(`/system/main-stories/${id}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${token}`,
      },
