...
export const adminAIModelsApi = {
  ...
  getAll: (token: string): Promise<AIModelConfig[]> => {
    return request<AIModelConfig[]>('/system/ai-config/models', {
      headers: {
  ...
  getByCapability: (capability: string, token: string): Promise<AIModelConfig[]> => {
    return request<AIModelConfig[]>(
      `/system/ai-config/models/${capability}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  getByProviderAndCapability: (
    ...
    return request<AIModelConfig[]>(
      `/system/ai-config/models/${capability}/${provider}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  update: (
    ...
    return request<AIModelConfig>(`/system/ai-config/models/${id}`, {
      method: 'PUT',
      body: JSON.stringify(data),
      headers: {
        Authorization: `Bearer ${token}`,
  ...
  delete: (id: number, token: string): Promise<void> => {
    return request<void>(`/system/ai-config/models/${id}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  setDefault: (
    ...
    return request<{
      ...
      capability: string;
      isDefault: boolean;
    }>(`/system/ai-config/models/${id}/set-default`, {
      method: 'PUT',
      headers: {
        Authorization: `Bearer ${token}`,
      },
...
export const adminAIRoutingStrategiesApi = {
  ...
  getAll: (token: string): Promise<RoutingStrategy[]> => {
    return request<RoutingStrategy[]>(
      '/system/ai-config/routing-strategies',
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  getByCapability: (capability: string, token: string): Promise<RoutingStrategy> => {
    return request<RoutingStrategy>(
      `/system/ai-config/routing-strategies/${capability}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  update: (
    ...
    return request<{
      ...
      config: any;
      isActive: boolean;
    }>(`/system/ai-config/routing-strategies/${id}`, {
      method: 'PUT',
      body: JSON.stringify(data),
      headers: {
        Authorization: `Bearer ${token}`,
   ...
   */
  delete: (id: number, token: string): Promise<void> => {
    return request<void>(`/system/ai-config/routing-strategies/${id}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${token}`,
      },
