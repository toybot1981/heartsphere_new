...
export const adminSubscriptionPlansApi = {
  ...
  getAll: async (token: string): Promise<SubscriptionPlan[]> => {
    try {
      return await request<SubscriptionPlan[]>('/system/subscription-plans', {
        headers: {
  ...
  getById: (id: number, token: string): Promise<SubscriptionPlan> => {
    return request<SubscriptionPlan>(`/system/subscription-plans/${id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
  ...
  create: (data: any, token: string): Promise<SubscriptionPlan> => {
    return request<SubscriptionPlan>('/system/subscription-plans', {
      method: 'POST',
  ...
  delete: (id: number, token: string): Promise<void> => {
    return request<void>(`/system/subscription-plans/${id}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${token}`,
      },
