...
export const adminEntityRelationApi = {
  ...
  createRelation: (relation: {
    ...
    return request<EntityRelation>('/entities/relations', {
      method: 'POST',
  ...
  getRelations: (entityType: string, entityId: string, token: string): Promise<EntityRelationListResponse> => {
    return request<EntityRelationListResponse>(`/entities/relations?entityType=${entityType}&entityId=${entityId}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
  ...
  getRelationsBetween: (
    ...
    return request<EntityRelationListResponse>(
      `/entities/relations/between?sourceType=${sourceType}&sourceId=${sourceId}&targetType=${targetType}&targetId=${targetId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  recommendRelatedEntities: (
    ...
    return request<{ items: any[]; total: number }>(
      `/entities/relations/recommend?entityType=${entityType}&entityId=${entityId}&limit=${limit}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  recommendPossibleRelations: (
    ...
    return request<{ items: EntityRelationRecommendation[]; total: number }>(
      `/entities/relations/recommend-relation?sourceType=${sourceType}&sourceId=${sourceId}&targetType=${targetType}&targetId=${targetId}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  updateRelationStrength: (relationId: number, strength: number, token: string): Promise<EntityRelation> => {
    return request<EntityRelation>(`/entities/relations/${relationId}/strength`, {
      method: 'PUT',
  ...
  deleteRelation: (relationId: number, token: string): Promise<void> => {
    return request<void>(`/entities/relations/${relationId}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${token}`,
      },
