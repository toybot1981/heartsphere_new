...
nohup npm run dev > "$PROJECT_ROOT/$PROJECT_DIR-frontend.log" 2>&1 &

PID=$!
echo -e "${GREEN}$PROJECT_NAME 已启动 (PID: $PID)${NC}"
echo -e "${YELLOW}日志文件: $PROJECT_ROOT/$PROJECT_DIR-frontend.log${NC}"
echo -e "${YELLOW}访问地址: http://localhost:$PORT${NC}"
echo ""

# 保存 PID
echo "$PID" > "$PROJECT_ROOT/$PROJECT_DIR-frontend.pid"
echo -e "${GREEN}PID 已保存到: $PROJECT_ROOT/$PROJECT_DIR-frontend.pid${NC}"
