...
# 加载端口管理工具（从 scripts/utils/ 目录）
source "$SCRIPT_DIR/../utils/port-utils.sh"

PORT=8081
PROJECT_NAME="主项目后端"
PROJECT_DIR="main/backend"
...
nohup mvn spring-boot:run -Pdev > "$PROJECT_ROOT/$PROJECT_DIR-backend.log" 2>&1 &

PID=$!
echo -e "${GREEN}$PROJECT_NAME 已启动 (PID: $PID)${NC}"
echo -e "${YELLOW}日志文件: $PROJECT_ROOT/$PROJECT_DIR-backend.log${NC}"
echo -e "${YELLOW}访问地址: http://localhost:$PORT${NC}"
echo ""

# 保存 PID
echo "$PID" > "$PROJECT_ROOT/$PROJECT_DIR-backend.pid"
echo -e "${GREEN}PID 已保存到: $PROJECT_ROOT/$PROJECT_DIR-backend.pid${NC}"
