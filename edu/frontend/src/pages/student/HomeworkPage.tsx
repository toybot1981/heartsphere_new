...
export const HomeworkPage: React.FC = () => {
  ...
  const homeworkList = mockHomework.filter(hw => hw.studentId === studentId);
      ...
      <div className="max-w-5xl mx-auto">
        <header className="mb-8">
          ...
          <h1 className={`text-4xl font-bold ${isElementary ? 'text-primary-elementary-700' : 'text-primary-middle-700'} mb-2`}>
            {isElementary ? '📚 我的作业' : '我的作业'}
          </h1>
          <p className="text-gray-600 text-lg">
            {isElementary ? '看看有哪些作业需要完成吧！' : '查看和管理你的作业'}
          </p>
        </header>
        ...
        {homeworkList.length === 0 ? (
          ...
          <div className="space-y-4">
            {homeworkList.map((homework) => (
              <Card 
                key={homework.id}
                onClick={() => navigate(`/student/homework/${homework.id}?ageGroup=${ageGroup}`)}
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <Button 
                    ageGroup={ageGroup}
                    variant={homework.status === 'pending' || homework.status === 'in_progress' ? 'primary' : 'outline'}
                    size="sm"
                  >
                    {homework.status === 'pending' || homework.status === 'in_progress' 
                      ? (isElementary ? '✏️ 开始做' : '开始做')
                      : homework.status === 'graded'
                      ? (isElementary ? '👀 查看批改' : '查看批改')
                      : (isElementary ? '👀 查看' : '查看')}
                  </Button>
                </div>
