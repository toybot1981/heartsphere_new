...
import type { Plugin, PluginListRequest, PluginListResponse, PluginConfigRequest } from './pluginTypes';
...
export const adminPluginApi = {
  ...
  getList: (params: PluginListRequest, token: string): Promise<PluginListResponse> => {
    ...
    return request<PluginListResponse>(`/admin/plugins?${queryParams.toString()}`, {
      headers: {
  ...
  getById: (pluginId: string, token: string): Promise<Plugin> => {
    return request<Plugin>(`/admin/plugins/${pluginId}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
  ...
  enable: (pluginId: string, token: string): Promise<void> => {
    return request<void>(`/admin/plugins/${pluginId}/enable`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  disable: (pluginId: string, force: boolean, token: string): Promise<void> => {
    return request<void>(`/admin/plugins/${pluginId}/disable?force=${force}`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  updateConfig: (pluginId: string, config: PluginConfigRequest, token: string): Promise<void> => {
    return request<void>(`/admin/plugins/${pluginId}/config`, {
      method: 'PUT',
  ...
  getPreview: (pluginId: string, token: string): Promise<PluginPreview> => {
    return request<PluginPreview>(`/admin/plugins/${pluginId}/preview`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
  ...
  publish: (pluginId: string, publishNote: string | undefined, token: string): Promise<void> => {
    return request<void>(`/admin/plugins/${pluginId}/publish`, {
      method: 'POST',
  ...
  unpublish: (pluginId: string, token: string): Promise<void> => {
    return request<void>(`/admin/plugins/${pluginId}/unpublish`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
      },
