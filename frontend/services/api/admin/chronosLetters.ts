...
export const adminChronosLettersApi = {
  ...
  getUserFeedbacks: (token: string): Promise<ChronosLetter[]> => {
    return request<ChronosLetter[]>('/admin/chronos-letters/user-feedbacks', {
      headers: {
  ...
  getLetterById: (letterId: string, token: string): Promise<ChronosLetter> => {
    return request<ChronosLetter>(`/admin/chronos-letters/${letterId}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
  ...
  replyToUserFeedback: (
    ...
    return request<ChronosLetter>(`/admin/chronos-letters/${letterId}/reply`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${token}`,
