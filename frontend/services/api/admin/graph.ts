...
import type { GraphDefinition, GraphNode, GraphEdge, GraphDefinitionCreateRequest } from './graphTypes';
import type { GraphExecutionRequest, GraphExecutionDTO } from './graphExecutionTypes';
...
export const adminGraphApi = {
  ...
  getAll: (token: string): Promise<GraphDefinition[]> => {
    return request<GraphDefinition[]>('/admin/graph', {
      headers: {
  ...
  getById: (id: number, token: string): Promise<GraphDefinition> => {
    return request<GraphDefinition>(`/admin/graph/${id}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
  ...
  delete: (id: number, token: string): Promise<void> => {
    return request<void>(`/admin/graph/${id}`, {
      method: 'DELETE',
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  execute: (id: number, executionRequest: GraphExecutionRequest, token: string): Promise<GraphExecutionDTO> => {
    return request<GraphExecutionDTO>(`/admin/graph/${id}/execute`, {
      method: 'POST',
  ...
  getExecutionStatus: (graphId: number, executionId: string, token: string): Promise<GraphExecutionDTO> => {
    return request<GraphExecutionDTO>(`/admin/graph/${graphId}/execution/${executionId}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
  ...
  getExecutionLogs: (executionId: string, token: string, all?: boolean): Promise<ExecutionLogDTO[]> => {
    const url = `/admin/graph/executions/${executionId}/logs${all ? '?all=true' : ''}`;
  ...
  submitChoice: (graphId: number, executionId: string, choiceRequest: GraphExecutionChoiceRequest, token: string): Promise<GraphExecutionDTO> => {
    return request<GraphExecutionDTO>(`/admin/graph/${graphId}/execution/${executionId}/choice`, {
      method: 'POST',
