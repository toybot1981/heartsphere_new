...
/**
 * 体验模式切面
 * 拦截数据保存操作，在体验模式下将数据保存到临时存储
 */
@Aspect
@Component
public class ExperienceModeAspect {
    
    private static final Logger log = LoggerFactory.getLogger(ExperienceModeAspect.class);
        ...
        if (ExperienceModeContext.isActive()) {
            ...
            if (shareConfigId != null && visitorId != null) {
                ...
                if (args.length > 0) {
                    ...
                    log.debug("体验模式：保存记忆到临时存储: shareConfigId={}, visitorId={}", 
        ...
        if (ExperienceModeContext.isActive()) {
            ...
            if (shareConfigId != null && visitorId != null) {
                ...
                if (args.length > 0) {
                    ...
                    log.debug("体验模式：保存情绪到临时存储: shareConfigId={}, visitorId={}", 
                        shareConfigId, visitorId);
                }
                return null;
            }
