...
public class JavaScriptScriptExecutor implements ScriptExecutor {
    ...
    public ScriptResult execute(String sessionId, String script, String language, 
        ...
        log.info("执行JavaScript脚本: sessionId={}, timeout={}s", sessionId, timeout);
        ...
        try {
            // 创建临时脚本文件
            scriptFile = Files.createTempFile("mentis_script_" + sessionId + "_", ".js");
            ...
            log.info("JavaScript脚本执行完成: sessionId={}, success={}, time={}ms", 
                    sessionId, result.isSuccess(), executionTime);
            
        } catch (Exception e) {
            log.error("执行JavaScript脚本失败: sessionId={}", sessionId, e);
            result.setSuccess(false);
    ...
    public boolean isLanguageSupported(String language) {
        return "javascript".equalsIgnoreCase(language) || 
               ...
               "nodejs".equalsIgnoreCase(language);
    }
