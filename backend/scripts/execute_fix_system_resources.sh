...
LOG_FILE="$LOG_DIR/fix_system_resources_$TIMESTAMP.log"

echo "=========================================="
echo "修复 system_resources 表中的 URL 路径"
echo "=========================================="
echo ""
...
if ! command -v mysql &> /dev/null; then
    ...
    echo ""
    echo "SQL 脚本位置: backend/scripts/fix_system_resources_urls.sql"
    exit 1
fi
...
echo ""
echo "开始修复 system_resources 表的 URL 路径..."
echo ""

# 执行修复脚本
mysql -h"$DB_HOST" -P"$DB_PORT" -u"$DB_USER" -p"$DB_PASSWORD" "$DB_NAME" \
    < backend/scripts/fix_system_resources_urls.sql \
    > "$LOG_FILE" 2>&1
...
if [ $? -eq 0 ]; then
    ...
    # 显示验证结果
    if grep -q "修复后检查\|status" "$LOG_FILE"; then
        echo "修复验证结果:"
        grep -A 30 "修复后检查" "$LOG_FILE" | head -40
    fi
else
    echo "修复失败"
