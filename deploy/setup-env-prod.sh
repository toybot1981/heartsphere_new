...
# 2. 提示设置缺失的环境变量
# 3. 支持强制更新所有环境变量
# 使用方法: ./setup-env-prod.sh [--force] [--check-only]
...
if [ "$CHECK_ONLY" = true ]; then
    ...
    if [ $missing_count -gt 0 ]; then
        ...
        echo "  ${CYAN}${SCRIPT_DIR}/setup-env-prod.sh${NC}"
        exit 1
    else
        echo ""
...
read_input "应用主目录" "${APP_HOME:-/opt/heartsphere}" "APP_HOME"
# 生产环境默认BASE_URL
BASE_URL="${BASE_URL:-http://heartsphere.cn}"
read_input "应用基础URL（例如: http://heartsphere.cn）" "${BASE_URL}" "BASE_URL"
read_input "后端端口" "${BACKEND_PORT:-8081}" "BACKEND_PORT"
read_input "前端端口" "${FRONTEND_PORT:-80}" "FRONTEND_PORT"

echo ""
echo -e "${BLUE}========== 图片存储配置 ==========${NC}"
read_input "图片存储类型 (local/oss/s3)" "${IMAGE_STORAGE_TYPE:-local}" "IMAGE_STORAGE_TYPE"
read_input "图片存储路径" "${IMAGE_STORAGE_PATH:-/opt/heartsphere/uploads/images}" "IMAGE_STORAGE_PATH"
...
echo -e "  1. 检查环境变量: ${CYAN}${SCRIPT_DIR}/setup-env-prod.sh --check-only${NC}"
echo -e "  2. 将 .env 文件上传到服务器: ${CYAN}scp ${ENV_FILE} user@server:/opt/heartsphere/.env${NC}"
echo -e "  3. 在服务器上加载环境变量: ${CYAN}source /opt/heartsphere/.env${NC}"
echo -e "  4. 开始部署: ${CYAN}${SCRIPT_DIR}/deploy-backend-prod.sh${NC}"
echo ""
