...
# 2. 提示设置缺失的环境变量
# 3. 支持强制更新所有环境变量
# 使用方法: ./setup-env.sh [--force] [--check-only]
...
EOF
    ...
    local categories=(
        ...
        "应用配置:APP_NAME,APP_HOME,BACKEND_PORT,FRONTEND_PORT"
...
if [ "$CHECK_ONLY" = true ]; then
    ...
    if [ $missing_count -gt 0 ]; then
        ...
        echo "  ${CYAN}${SCRIPT_DIR}/setup-env.sh${NC}"
        exit 1
    else
        echo ""
...
read_input "图片服务基础URL（留空使用默认）" "${IMAGE_BASE_URL:-}" "IMAGE_BASE_URL"
...
echo -e "  1. 检查环境变量: ${CYAN}${SCRIPT_DIR}/setup-env.sh --check-only${NC}"
echo -e "  2. 加载环境变量: ${CYAN}source ${ENV_FILE}${NC}"
echo -e "  3. 开始部署: ${CYAN}${SCRIPT_DIR}/deploy-backend.sh${NC}"
echo ""
