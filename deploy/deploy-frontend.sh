...
# 2. 安装 Nginx
echo -e "${YELLOW}[2/7] 检查 Nginx 环境...${NC}"
...
EOF
    ...
    echo -e "${YELLOW}警告: 未找到 ${ENV_FILE} 文件，将使用默认配置${NC}"
fi
...
if systemctl is-active --quiet nginx; then
    ...
    echo -e "${GREEN}查看日志: journalctl -u nginx -f${NC}"
else
