# 记忆系统与温度感系统集成说明

**文档版本**: V1.0  
**编写日期**: 2025-12-28

---

## 📋 概述

本文档说明记忆系统如何与温度感系统集成，实现情感记忆的自动提取、存储和检索。

---

## 🔗 集成架构

### 1. 核心组件

```
温度感系统                   记忆系统
┌─────────────┐            ┌─────────────┐
│EmotionService│───────────▶│Temperature  │
│             │             │MemoryService│
└─────────────┘             └──────┬──────┘
                                    │
                                    ▼
                            ┌─────────────┐
                            │MemoryManager│
                            └──────┬──────┘
                                   │
                    ┌──────────────┼──────────────┐
                    ▼              ▼              ▼
            ┌──────────┐  ┌──────────┐  ┌──────────┐
            │ShortMemory│  │LongMemory│  │Extractor │
            │  Service  │  │  Service │  │          │
            └──────────┘  └──────────┘  └──────────┘
```

### 2. 数据流

1. **情绪记录保存** → `EmotionService.saveEmotionRecord()`
2. **自动提取记忆** → `TemperatureMemoryService.extractMemoriesFromEmotion()`
3. **保存到长期记忆** → `MongoLongMemoryService.saveMemory()`
4. **检索相关记忆** → `TemperatureMemoryService.retrieveMemoriesByEmotion()`

---

## 🎯 支持的情感记忆类型

### 1. 情绪模式记忆 (EMOTION_PATTERN)

**触发条件**：
- 置信度 > 0.7
- 情绪强度为 moderate 或 strong

**存储内容**：
- 情绪类型
- 情绪强度
- 置信度
- 时间戳

**用途**：
- 识别用户的情绪模式
- 预测用户可能的情绪状态

### 2. 情感经历记忆 (EMOTIONAL_EXPERIENCE)

**触发条件**：
- 情绪强度为 strong
- 置信度 > 0.7

**存储内容**：
- 情绪类型
- 触发文本
- 上下文信息
- 时间戳

**用途**：
- 记录用户的重要情感经历
- 用于个性化回应生成

### 3. 情绪偏好记忆 (EMOTIONAL_PREFERENCE)

**触发条件**：
- 积极情绪（happy, excited, content等）
- 关键短语包含偏好相关词汇

**存储内容**：
- 情绪类型
- 关键短语
- 偏好信息

**用途**：
- 识别用户的情感偏好
- 优化交互体验

---

## 💻 使用方式

### 1. 自动提取（已集成）

在 `EmotionService.saveEmotionRecord()` 中自动调用：

```java
@Autowired
private TemperatureMemoryService temperatureMemoryService;

public EmotionRecord saveEmotionRecord(EmotionAnalysisResponse analysis, 
                                       EmotionAnalysisRequest request) {
    // 保存情绪记录
    EmotionRecord record = emotionRecordRepository.save(emotionRecord);
    
    // 自动提取记忆
    try {
        temperatureMemoryService.extractMemoriesFromEmotion(
            String.valueOf(request.getUserId()), 
            record
        );
    } catch (Exception e) {
        log.error("提取记忆失败", e);
        // 不影响主流程
    }
    
    return record;
}
```

### 2. 手动检索记忆

```java
@Autowired
private TemperatureMemoryService temperatureMemoryService;

// 根据情绪检索相关记忆
List<UserMemory> memories = temperatureMemoryService.retrieveMemoriesByEmotion(
    userId, 
    "happy",  // 情绪类型
    10        // 返回数量
);

// 获取情绪模式
List<UserMemory> patterns = temperatureMemoryService.getEmotionPatterns(userId);

// 获取情感经历
List<UserMemory> experiences = temperatureMemoryService.getEmotionalExperiences(
    userId, 
    "sad",    // 可选的情绪类型
    20        // 返回数量
);
```

### 3. 分析情绪趋势

```java
// 分析并保存情绪趋势
UserMemory trend = temperatureMemoryService.analyzeAndSaveEmotionTrend(
    userId, 
    "week"    // 时间段：day/week/month
);
```

---

## 🔧 配置说明

### 1. 记忆提取阈值

在 `application.yml` 中配置：

```yaml
heartsphere:
  memory:
    long-memory:
      extraction-importance-threshold: 0.7  # 重要性阈值
      extraction-confidence-threshold: 0.6   # 置信度阈值
```

### 2. MongoDB索引

已创建以下索引支持情感记忆检索：

```javascript
// user_memories集合索引
db.user_memories.createIndex({ userId: 1, type: 1 });
db.user_memories.createIndex({ userId: 1, "metadata.emotionType": 1 });
db.user_memories.createIndex({ content: "text" });
```

---

## 📊 数据结构

### UserMemory结构（情感记忆）

```java
UserMemory {
    userId: String
    type: MemoryType (EMOTION_PATTERN | EMOTIONAL_EXPERIENCE | EMOTIONAL_PREFERENCE)
    importance: MemoryImportance (CORE | IMPORTANT | NORMAL | TEMPORARY)
    content: String
    structuredData: Map {
        emotionType: String
        intensity: String
        confidence: Double
        triggerText: String (可选)
        keyPhrases: String (可选)
    }
    metadata: Map {
        emotionType: String  // 用于检索
        timestamp: String
        source: String
    }
    source: MemorySource (SYSTEM_DETECTED)
    confidence: Double
    tags: List<String>
}
```

---

## 🎨 使用场景

### 场景1：个性化回应生成

```java
// 1. 检测到用户情绪
EmotionAnalysisResponse emotion = emotionService.analyzeEmotion(request);

// 2. 检索相关记忆
List<UserMemory> memories = temperatureMemoryService.retrieveMemoriesByEmotion(
    userId, 
    emotion.getEmotionType(), 
    5
);

// 3. 基于记忆生成个性化回应
String response = generatePersonalizedResponse(emotion, memories);
```

### 场景2：情绪模式识别

```java
// 获取用户的情绪模式
List<UserMemory> patterns = temperatureMemoryService.getEmotionPatterns(userId);

// 分析模式
Map<String, Integer> emotionFrequency = new HashMap<>();
for (UserMemory pattern : patterns) {
    String emotionType = (String) pattern.getMetadata().get("emotionType");
    emotionFrequency.put(emotionType, 
        emotionFrequency.getOrDefault(emotionType, 0) + 1);
}

// 识别主要情绪模式
String dominantEmotion = emotionFrequency.entrySet().stream()
    .max(Map.Entry.comparingByValue())
    .map(Map.Entry::getKey)
    .orElse("calm");
```

### 场景3：情绪趋势分析

```java
// 分析周趋势
UserMemory weekTrend = temperatureMemoryService.analyzeAndSaveEmotionTrend(
    userId, 
    "week"
);

// 获取趋势数据
Map<String, Object> trendData = weekTrend.getStructuredData();
String dominantEmotion = (String) trendData.get("dominantEmotion");
Double avgConfidence = (Double) trendData.get("averageConfidence");
```

---

## 🔍 检索示例

### 1. 根据情绪类型检索

```java
// 检索与"happy"相关的所有记忆
List<UserMemory> memories = mongoLongMemoryService.retrieveMemoriesByContext(
    userId,
    Map.of(
        "emotionType", "happy"
    ),
    10
);
```

### 2. 检索特定类型的记忆

```java
// 检索所有情绪模式记忆
List<UserMemory> patterns = mongoLongMemoryService.retrieveMemoriesByContext(
    userId,
    Map.of(
        "type", "EMOTION_PATTERN"
    ),
    20
);
```

### 3. 组合检索

```java
// 检索特定情绪类型的重要记忆
List<UserMemory> memories = mongoLongMemoryService.retrieveMemoriesByContext(
    userId,
    Map.of(
        "type", "EMOTIONAL_EXPERIENCE",
        "importance", "IMPORTANT",
        "emotionType", "sad"
    ),
    10
);
```

---

## ⚠️ 注意事项

### 1. 异步处理

记忆提取是异步进行的，不会阻塞情绪记录保存的主流程。

### 2. 错误处理

如果记忆提取失败，不会影响情绪记录的保存，只记录错误日志。

### 3. 性能考虑

- 记忆提取使用异步处理
- 批量保存记忆以提高性能
- 使用MongoDB索引优化检索

### 4. 数据一致性

- 情绪记录和记忆之间通过sourceId关联
- 支持通过sourceId追溯原始情绪记录

---

## 📈 未来扩展

### 1. 记忆关联

- 建立记忆之间的关联关系
- 支持通过关联检索相关记忆

### 2. 记忆衰减

- 实现记忆重要性衰减机制
- 自动清理低重要性记忆

### 3. 记忆可视化

- 提供记忆可视化接口
- 展示用户的情绪模式和趋势

---

## 🔗 相关文档

- [阶段一详细设计](./02-阶段一：基础记忆系统.md)
- [长期记忆与短期记忆设计详解](./长期记忆与短期记忆设计详解.md)
- [API文档](./07-API文档.md)

---

**集成完成！** ✅




