# 阶段四和阶段五使用指南

**版本**: v1.0  
**日期**: 2025-12-29

---

## 📋 目录

- [概述](#概述)
- [快速开始](#快速开始)
- [功能使用](#功能使用)
- [最佳实践](#最佳实践)
- [常见问题](#常见问题)

---

## 概述

阶段四和阶段五为记忆系统提供了高级功能和优化能力：

### 阶段四：高级记忆能力

- **向量搜索**: 基于语义相似度的记忆检索
- **记忆关联**: 自动发现和管理记忆之间的关联关系
- **智能检索**: 多维度、智能排序的记忆检索
- **记忆巩固**: 将短期记忆转换为长期记忆

### 阶段五：记忆系统优化

- **缓存优化**: 多级缓存提升性能
- **压缩归档**: 节省存储空间
- **监控诊断**: 系统健康监控和性能分析

---

## 快速开始

### 1. 环境准备

#### 依赖配置

确保 `pom.xml` 中包含以下依赖：

```xml
<!-- Caffeine Cache -->
<dependency>
    <groupId>com.github.ben-manes.caffeine</groupId>
    <artifactId>caffeine</artifactId>
</dependency>
```

#### 配置文件

在 `application.yml` 中配置：

```yaml
spring:
  ai:
    dashscope:
      api-key: your-api-key
      base-url: https://dashscope.aliyuncs.com/compatible-mode/v1
  data:
    redis:
      host: localhost
      port: 6379
    mongodb:
      uri: mongodb://localhost:27017/heartsphere
```

### 2. 基本使用

#### 生成向量嵌入

```java
@Autowired
private DashScopeEmbeddingService embeddingService;

// 生成单个文本的向量
float[] embedding = embeddingService.generateEmbedding("测试文本");

// 批量生成向量
List<String> texts = Arrays.asList("文本1", "文本2", "文本3");
List<float[]> embeddings = embeddingService.generateEmbeddings(texts);
```

#### 向量搜索

```java
@Autowired
private VectorSearchService vectorSearchService;

// 语义搜索
List<VectorSearchService.SimilarMemory> results = 
    vectorSearchService.searchSimilarMemories(
        "查询文本",
        "user-1",
        null,
        null,
        10,
        0.6
    );
```

#### 使用缓存

```java
@Autowired
private MemoryCacheService cacheService;

// 写入缓存
cacheService.put("memory:1", memoryObject, Duration.ofHours(1));

// 读取缓存
Optional<Memory> memory = cacheService.get("memory:1", Memory.class);

// 删除缓存
cacheService.evict("memory:1");
```

---

## 功能使用

### 1. 向量搜索

#### 使用场景

- 语义相似度搜索
- 相关记忆推荐
- 上下文记忆检索

#### 示例代码

```java
// 1. 生成查询向量
float[] queryVector = vectorSearchService.generateEmbedding("用户喜欢什么");

// 2. 搜索相似记忆
List<VectorSearchService.SimilarMemory> similarMemories = 
    vectorSearchService.searchSimilarMemories(
        "用户喜欢什么",
        userId,
        null,
        null,
        10,
        0.6  // 相似度阈值
    );

// 3. 处理结果
for (VectorSearchService.SimilarMemory memory : similarMemories) {
    System.out.println("记忆ID: " + memory.getMemoryId());
    System.out.println("相似度: " + memory.getSimilarity());
    System.out.println("内容: " + memory.getContent());
}
```

### 2. 记忆关联

#### 使用场景

- 发现相关记忆
- 构建记忆网络
- 关联关系分析

#### 示例代码

```java
@Autowired
private MemoryAssociationService associationService;

// 1. 发现记忆关联
List<MemoryAssociationService.MemoryAssociation> associations = 
    associationService.discoverAssociations("memory-1", 10);

// 2. 获取记忆的所有关联
List<MemoryAssociationService.MemoryAssociation> allAssociations = 
    associationService.getAssociations("memory-1");

// 3. 基于关联检索记忆
List<MemoryAssociationService.MemoryAssociation> relatedMemories = 
    associationService.retrieveByAssociation("memory-1", 10);
```

### 3. 智能检索

#### 使用场景

- 多维度记忆检索
- 智能排序和推荐
- 上下文相关检索

#### 示例代码

```java
@Autowired
private IntelligentRetrievalService retrievalService;

// 1. 多维度检索
List<IntelligentRetrievalService.RetrievedMemory> results = 
    retrievalService.multiDimensionSearch(
        "查询文本",
        userId,
        characterId,
        participantId,
        context,
        10
    );

// 2. 混合检索（可配置权重）
List<IntelligentRetrievalService.RetrievedMemory> hybridResults = 
    retrievalService.hybridSearch(
        "查询文本",
        userId,
        0.4,  // 语义权重
        0.4,  // 关键词权重
        0.2,  // 关联权重
        10
    );
```

### 4. 缓存优化

#### 使用场景

- 提升读取性能
- 减少数据库压力
- 缓存热点数据

#### 示例代码

```java
@Autowired
private MemoryCacheService cacheService;

// 1. 缓存预热
List<String> hotMemoryIds = Arrays.asList("memory-1", "memory-2", "memory-3");
cacheService.warmup(hotMemoryIds);

// 2. 获取缓存统计
MemoryCacheService.CacheStats stats = cacheService.getStats();
System.out.println("L1命中率: " + stats.getL1HitRate());
System.out.println("L2命中率: " + stats.getL2HitRate());
System.out.println("总命中率: " + stats.getTotalHitRate());

// 3. 清理缓存
cacheService.clear("memory");  // 清理指定类型
cacheService.clear(null);      // 清理所有缓存
```

### 5. 压缩归档

#### 使用场景

- 节省存储空间
- 归档长期未使用的记忆
- 管理记忆生命周期

#### 示例代码

```java
@Autowired
private MemoryCompressionService compressionService;
@Autowired
private MemoryArchivingService archivingService;

// 1. 压缩记忆
List<String> memoryIds = Arrays.asList("memory-1", "memory-2");
List<MemoryCompressionService.CompressionResult> results = 
    compressionService.compressBatch(memoryIds);

// 2. 归档记忆
List<MemoryArchivingService.ArchiveResult> archiveResults = 
    archivingService.archiveBatch(memoryIds, "长期未使用");

// 3. 查询归档列表
Pageable pageable = PageRequest.of(0, 20);
Page<ArchivedMemory> archivedPage = 
    archivingService.listArchived(userId, pageable);

// 4. 恢复归档记忆
archivingService.restore("archived-1");
```

### 6. 监控诊断

#### 使用场景

- 系统健康监控
- 性能分析
- 问题诊断

#### 示例代码

```java
@Autowired
private MemoryMonitoringService monitoringService;

// 1. 健康检查
MemoryMonitoringService.HealthStatus health = monitoringService.healthCheck();
System.out.println("系统状态: " + health.getStatus());
for (Map.Entry<String, ComponentHealth> entry : health.getComponents().entrySet()) {
    System.out.println(entry.getKey() + ": " + entry.getValue().getStatus());
}

// 2. 获取性能指标
Instant startTime = Instant.now().minusSeconds(3600);
Instant endTime = Instant.now();
MemoryMonitoringService.PerformanceMetrics metrics = 
    monitoringService.getPerformanceMetrics(startTime, endTime);
System.out.println("P95响应时间: " + metrics.getP95ResponseTime() + "ms");
System.out.println("QPS: " + metrics.getQps());
System.out.println("缓存命中率: " + metrics.getCacheHitRate());

// 3. 获取诊断信息
MemoryMonitoringService.DiagnosticsInfo diagnostics = 
    monitoringService.diagnose();
System.out.println("健康状态: " + diagnostics.getHealth().getStatus());
for (String recommendation : diagnostics.getRecommendations()) {
    System.out.println("建议: " + recommendation);
}
```

---

## 最佳实践

### 1. Embedding 使用

**建议**:
- 批量处理时使用 `generateEmbeddings()` 而不是多次调用 `generateEmbedding()`
- 对于大量文本，考虑异步处理
- 实现本地缓存避免重复调用

**示例**:
```java
// 好的做法：批量处理
List<String> texts = getTexts();
List<float[]> embeddings = embeddingService.generateEmbeddings(texts);

// 避免：循环调用
for (String text : texts) {
    float[] embedding = embeddingService.generateEmbedding(text);  // 不推荐
}
```

### 2. 缓存使用

**建议**:
- 合理设置TTL，平衡性能和一致性
- 使用缓存预热提升性能
- 定期清理过期缓存

**示例**:
```java
// 设置合理的TTL
cacheService.put(key, value, Duration.ofHours(1));  // 热点数据
cacheService.put(key, value, Duration.ofMinutes(5)); // 临时数据

// 缓存预热
List<String> hotKeys = getHotMemoryIds();
cacheService.warmup(hotKeys);
```

### 3. 压缩归档

**建议**:
- 只压缩低频访问的记忆
- 定期归档长期未使用的记忆
- 保留重要记忆不压缩

**示例**:
```java
// 识别低频访问的记忆
List<String> lowFrequencyMemories = findLowFrequencyMemories(userId);

// 压缩
compressionService.compressBatch(lowFrequencyMemories);

// 归档长期未使用的记忆
List<String> oldMemories = findOldMemories(userId, 90); // 90天前
archivingService.archiveBatch(oldMemories, "长期未使用");
```

### 4. 监控诊断

**建议**:
- 定期检查系统健康状态
- 监控关键性能指标
- 根据诊断建议优化系统

**示例**:
```java
// 定时健康检查
@Scheduled(fixedRate = 60000) // 每分钟
public void healthCheck() {
    HealthStatus health = monitoringService.healthCheck();
    if (!"HEALTHY".equals(health.getStatus())) {
        log.warn("系统健康状态异常: {}", health.getStatus());
        // 发送告警
    }
}
```

---

## 常见问题

### Q1: Embedding API 调用失败怎么办？

**A**: 系统实现了降级策略，API失败时会自动使用哈希向量。检查：
1. API Key是否正确配置
2. 网络连接是否正常
3. API调用是否超限

### Q2: 缓存命中率低怎么办？

**A**: 可以尝试：
1. 增加缓存预热
2. 调整缓存TTL
3. 检查缓存键设计是否合理

### Q3: 压缩率不理想怎么办？

**A**: 考虑：
1. 检查内容是否适合压缩（文本压缩效果好）
2. 调整压缩阈值
3. 只压缩低频访问的记忆

### Q4: 如何监控系统性能？

**A**: 使用监控API：
1. 定期调用 `/api/memory/v5/monitoring/metrics` 获取性能指标
2. 调用 `/api/memory/v5/monitoring/health` 检查健康状态
3. 调用 `/api/memory/v5/monitoring/diagnostics` 获取诊断信息

### Q5: 归档的记忆如何恢复？

**A**: 使用恢复API：
```bash
POST /api/memory/v5/archiving/restore?archivedMemoryId=archived-1
```

---

## 📚 相关文档

- [API文档](./阶段四和阶段五API文档.md)
- [技术方案](../开发/阶段四和阶段五技术方案.md)
- [完成报告](../开发/阶段四和阶段五完成报告.md)

---

**文档版本**: v1.0  
**最后更新**: 2025-12-29


