# Memory数据库表创建说明

**日期**: 2025-12-31  
**状态**: ✅ 实体类已创建

---

## 📋 数据库表列表

### 短期记忆表

#### 1. `chat_messages` - 对话消息表
**实体类**: `ChatMessageEntity`

**字段**:
- `id` (VARCHAR(64), PK) - 消息ID
- `session_id` (VARCHAR(64), NOT NULL) - 会话ID
- `user_id` (VARCHAR(64), NOT NULL) - 用户ID
- `role` (VARCHAR(20), NOT NULL) - 消息角色（USER/ASSISTANT/SYSTEM）
- `content` (TEXT, NOT NULL) - 消息内容
- `metadata` (JSON) - 元数据（JSON格式）
- `timestamp` (BIGINT, NOT NULL) - 时间戳（毫秒）
- `importance` (VARCHAR(20)) - 重要性
- `created_at` (DATETIME) - 创建时间
- `updated_at` (DATETIME) - 更新时间
- `expires_at` (DATETIME) - 过期时间

**索引**:
- `idx_session_id` - session_id
- `idx_user_id` - user_id
- `idx_timestamp` - timestamp
- `idx_session_timestamp` - (session_id, timestamp)

#### 2. `working_memories` - 工作记忆表
**实体类**: `WorkingMemoryEntity`

**字段**:
- `id` (BIGINT, PK, AUTO_INCREMENT) - 主键
- `session_id` (VARCHAR(64), NOT NULL) - 会话ID
- `memory_key` (VARCHAR(100), NOT NULL) - 记忆键
- `memory_value` (TEXT) - 记忆值（JSON格式）
- `created_at` (DATETIME) - 创建时间
- `updated_at` (DATETIME) - 更新时间
- `expires_at` (DATETIME) - 过期时间

**索引**:
- `idx_session_id` - session_id
- `idx_session_key` - (session_id, memory_key) UNIQUE
- `idx_expires_at` - expires_at

#### 3. `memory_sessions` - 会话索引表
**实体类**: `SessionEntity`

**字段**:
- `id` (BIGINT, PK, AUTO_INCREMENT) - 主键
- `session_id` (VARCHAR(64), NOT NULL, UNIQUE) - 会话ID
- `user_id` (VARCHAR(64), NOT NULL) - 用户ID
- `created_at` (DATETIME) - 创建时间
- `updated_at` (DATETIME) - 更新时间
- `expires_at` (DATETIME) - 过期时间

**索引**:
- `idx_user_id` - user_id
- `idx_session_id` - session_id (UNIQUE)
- `idx_updated_at` - updated_at

### 长期记忆表

#### 4. `user_memories` - 用户记忆表
**实体类**: `UserMemoryEntity`

**字段**:
- `id` (VARCHAR(64), PK) - 记忆ID
- `user_id` (VARCHAR(64), NOT NULL) - 用户ID
- `type` (VARCHAR(50), NOT NULL) - 记忆类型（枚举）
- `importance` (VARCHAR(20), NOT NULL) - 重要性（枚举）
- `content` (TEXT, NOT NULL) - 记忆内容
- `structured_data` (JSON) - 结构化数据（JSON格式）
- `source` (VARCHAR(30), NOT NULL) - 记忆来源（枚举）
- `source_id` (VARCHAR(64)) - 来源ID
- `created_at` (DATETIME) - 创建时间
- `last_accessed_at` (DATETIME) - 最后访问时间
- `access_count` (INT, DEFAULT 0) - 访问次数
- `confidence` (DOUBLE) - 提取置信度
- `tags` (JSON) - 标签（JSON数组格式）
- `metadata` (JSON) - 扩展元数据（JSON格式）
- `updated_at` (DATETIME) - 更新时间

**索引**:
- `idx_user_id` - user_id
- `idx_type` - type
- `idx_importance` - importance
- `idx_source` - source
- `idx_source_id` - source_id
- `idx_created_at` - created_at
- `idx_user_type` - (user_id, type)
- `idx_user_importance` - (user_id, importance)

#### 5. `user_facts` - 用户事实表
**实体类**: `UserFactEntity`

**字段**:
- `id` (VARCHAR(64), PK) - 事实ID
- `user_id` (VARCHAR(64), NOT NULL) - 用户ID
- `fact` (TEXT, NOT NULL) - 事实内容
- `category` (VARCHAR(30), NOT NULL) - 事实类别（枚举）
- `importance` (DOUBLE, DEFAULT 0.5) - 重要性（0-1）
- `confidence` (DOUBLE, DEFAULT 0.7) - 置信度（0-1）
- `source_session_id` (VARCHAR(64)) - 来源会话ID
- `created_at` (DATETIME) - 创建时间
- `last_accessed_at` (DATETIME) - 最后访问时间
- `access_count` (INT, DEFAULT 0) - 访问次数
- `tags` (JSON) - 标签（JSON数组格式）
- `metadata` (JSON) - 元数据（JSON格式）
- `updated_at` (DATETIME) - 更新时间

**索引**:
- `idx_user_id` - user_id
- `idx_category` - category
- `idx_user_category` - (user_id, category)

#### 6. `user_preferences` - 用户偏好表
**实体类**: `UserPreferenceEntity`

**字段**:
- `id` (VARCHAR(64), PK) - 偏好ID
- `user_id` (VARCHAR(64), NOT NULL) - 用户ID
- `preference_key` (VARCHAR(100), NOT NULL) - 偏好键
- `preference_value` (TEXT, NOT NULL) - 偏好值
- `preference_type` (VARCHAR(30)) - 偏好类型（枚举）
- `created_at` (DATETIME) - 创建时间
- `updated_at` (DATETIME) - 更新时间

**索引**:
- `idx_user_id` - user_id
- `idx_key` - preference_key
- `idx_user_key` - (user_id, preference_key) UNIQUE

---

## 🔧 表创建方式

### 方式一：自动创建（推荐用于开发环境）

如果使用 `application.properties` 配置：
```properties
spring.jpa.hibernate.ddl-auto=update
```

**说明**：
- JPA会在应用启动时自动创建表
- 如果表已存在，会自动更新表结构
- **适用于开发环境**

### 方式二：手动创建（推荐用于生产环境）

如果使用 `application.yml` 配置：
```yaml
spring:
  jpa:
    hibernate:
      ddl-auto: validate  # 只验证，不创建
```

**说明**：
- 需要手动执行SQL脚本创建表
- **适用于生产环境**

---

## 📝 SQL创建脚本

如果需要手动创建表，可以使用以下SQL脚本：

```sql
-- 1. 对话消息表
CREATE TABLE IF NOT EXISTS `chat_messages` (
  `id` VARCHAR(64) NOT NULL PRIMARY KEY,
  `session_id` VARCHAR(64) NOT NULL,
  `user_id` VARCHAR(64) NOT NULL,
  `role` VARCHAR(20) NOT NULL,
  `content` TEXT NOT NULL,
  `metadata` JSON,
  `timestamp` BIGINT NOT NULL,
  `importance` VARCHAR(20),
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `expires_at` DATETIME,
  INDEX `idx_session_id` (`session_id`),
  INDEX `idx_user_id` (`user_id`),
  INDEX `idx_timestamp` (`timestamp`),
  INDEX `idx_session_timestamp` (`session_id`, `timestamp`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- 2. 工作记忆表
CREATE TABLE IF NOT EXISTS `working_memories` (
  `id` BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  `session_id` VARCHAR(64) NOT NULL,
  `memory_key` VARCHAR(100) NOT NULL,
  `memory_value` TEXT,
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `expires_at` DATETIME,
  INDEX `idx_session_id` (`session_id`),
  UNIQUE KEY `idx_session_key` (`session_id`, `memory_key`),
  INDEX `idx_expires_at` (`expires_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- 3. 会话索引表
CREATE TABLE IF NOT EXISTS `memory_sessions` (
  `id` BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,
  `session_id` VARCHAR(64) NOT NULL UNIQUE,
  `user_id` VARCHAR(64) NOT NULL,
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `expires_at` DATETIME,
  INDEX `idx_user_id` (`user_id`),
  INDEX `idx_updated_at` (`updated_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- 4. 用户记忆表
CREATE TABLE IF NOT EXISTS `user_memories` (
  `id` VARCHAR(64) NOT NULL PRIMARY KEY,
  `user_id` VARCHAR(64) NOT NULL,
  `type` VARCHAR(50) NOT NULL,
  `importance` VARCHAR(20) NOT NULL,
  `content` TEXT NOT NULL,
  `structured_data` JSON,
  `source` VARCHAR(30) NOT NULL,
  `source_id` VARCHAR(64),
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
  `last_accessed_at` DATETIME,
  `access_count` INT DEFAULT 0,
  `confidence` DOUBLE,
  `tags` JSON,
  `metadata` JSON,
  `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  INDEX `idx_user_id` (`user_id`),
  INDEX `idx_type` (`type`),
  INDEX `idx_importance` (`importance`),
  INDEX `idx_source` (`source`),
  INDEX `idx_source_id` (`source_id`),
  INDEX `idx_created_at` (`created_at`),
  INDEX `idx_user_type` (`user_id`, `type`),
  INDEX `idx_user_importance` (`user_id`, `importance`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- 5. 用户事实表
CREATE TABLE IF NOT EXISTS `user_facts` (
  `id` VARCHAR(64) NOT NULL PRIMARY KEY,
  `user_id` VARCHAR(64) NOT NULL,
  `fact` TEXT NOT NULL,
  `category` VARCHAR(30) NOT NULL,
  `importance` DOUBLE DEFAULT 0.5,
  `confidence` DOUBLE DEFAULT 0.7,
  `source_session_id` VARCHAR(64),
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
  `last_accessed_at` DATETIME,
  `access_count` INT DEFAULT 0,
  `tags` JSON,
  `metadata` JSON,
  `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  INDEX `idx_user_id` (`user_id`),
  INDEX `idx_category` (`category`),
  INDEX `idx_user_category` (`user_id`, `category`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- 6. 用户偏好表
CREATE TABLE IF NOT EXISTS `user_preferences` (
  `id` VARCHAR(64) NOT NULL PRIMARY KEY,
  `user_id` VARCHAR(64) NOT NULL,
  `preference_key` VARCHAR(100) NOT NULL,
  `preference_value` TEXT NOT NULL,
  `preference_type` VARCHAR(30),
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  INDEX `idx_user_id` (`user_id`),
  INDEX `idx_key` (`preference_key`),
  UNIQUE KEY `idx_user_key` (`user_id`, `preference_key`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
```

---

## ✅ 验证表是否已创建

### 方法一：检查数据库

```sql
-- 查看所有memory相关的表
SHOW TABLES LIKE '%memory%';
SHOW TABLES LIKE '%chat%';

-- 或者查看所有表
SHOW TABLES;
```

### 方法二：检查应用日志

启动应用时，如果看到以下日志，说明表已创建：
```
Hibernate: create table chat_messages ...
Hibernate: create table user_memories ...
...
```

### 方法三：检查表结构

```sql
-- 查看表结构
DESCRIBE chat_messages;
DESCRIBE user_memories;
DESCRIBE user_facts;
DESCRIBE user_preferences;
DESCRIBE working_memories;
DESCRIBE memory_sessions;
```

---

## 🔍 当前状态

### ✅ 已完成
- [x] 6个实体类已创建
- [x] 所有实体类都有 `@Table` 注解指定表名
- [x] 所有实体类都有必要的索引定义
- [x] JPA配置已设置（`ddl-auto=update`）

### ⏳ 待验证
- [ ] 数据库表是否已实际创建
- [ ] 表结构是否正确
- [ ] 索引是否已创建

---

## 💡 建议

1. **开发环境**：使用 `ddl-auto=update`，让JPA自动创建表
2. **生产环境**：使用 `ddl-auto=validate`，手动执行SQL脚本创建表
3. **验证**：启动应用后，检查数据库确认表已创建

---

**最后更新**: 2025-12-31  
**文档维护**: HeartSphere 开发团队

